...
function BuZhenNewLayer:onCreate(param)
    ...
    mm.GuildScene.zhanBtn = self.zhandouBtn
...
function BuZhenNewLayer:onEnter()
    print("mm.GuildId  BuZhenNewLayer    "..mm.GuildId)

    local HeroNum = #mm.data.playerHero
    if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
        mm.GuildId = 999999
        Guide:GuildEnd()
    end
    ...
    if mm.GuildId == 10033 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(10034, self.heroItemBtn)

        end, 0.01)
    elseif mm.GuildId == 10202 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(10203, self.imageDibtn)
        end, 0.01)
    elseif mm.GuildId == 10305 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(10306, self.imageDibtn)
        end, 0.01)
    elseif mm.GuildId == 10405 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(10406, self.imageDibtn)
        end, 0.01)
    elseif mm.GuildId == 10605 then
        local HeroNum = #mm.data.playerHero
        local canup = 0
        ...
        if canup <= 0 then
            ...
            performWithDelay(self,function( ... )
                Guide:startGuildById(10606, self.imageDibtn)
            end, 0.01)
        end
    elseif mm.GuildId == 15001 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(15002, self.imageDibtn)

        end, 0.01)
    elseif mm.GuildId == 15051 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(15052, self.imageDibtn)

        end, 0.01)
    end

    if mm.GuildId == 80006 then
        performWithDelay(self,function( ... )
            Guide:startGuildById(80007, self.imageDibtn)
...
function BuZhenNewLayer:updateMyZhen( widget, touchkey)
    if touchkey == ccui.TouchEventType.ended then
        ...
        if mm.GuildId == 10034 then
            performWithDelay(self,function( ... )
                -- Guide:startGuildById(10035, self.heroZhenImg)
                Guide:startGuildById(10036, mm.GuildScene.zhanBtn)
            end, 0.01)
        elseif mm.GuildId == 10203 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(10204, mm.GuildScene.zhanBtn)
            end
        elseif mm.GuildId == 10306 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(10307, mm.GuildScene.zhanBtn)
            end
        elseif mm.GuildId == 10406 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(10407, mm.GuildScene.zhanBtn)
            end
        elseif mm.GuildId == 10606 then
            local HeroNum = #mm.data.playerHero
            local canup = 0
            for k,v in pairs(mm.data.playerHero) do
                if self:canShow(v) == true then
            ...
            if canup <= 0 then
                ...
                if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum or #self.myZhen >= canup then
                    Guide:startGuildById(10607, mm.GuildScene.zhanBtn)
                end
            end
        elseif mm.GuildId == 15002 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(15003, mm.GuildScene.zhanBtn)
            end
        elseif mm.GuildId == 15052 then
            Guide:startGuildById(15053, self.imageDibtn)
        elseif mm.GuildId == 15053 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(15054, mm.GuildScene.zhanBtn)
            end
        end

        if mm.GuildId == 80007 then
            local HeroNum = #mm.data.playerHero
            if (#self.myZhen >= GuildNum) or #self.myZhen >= HeroNum then
                Guide:startGuildById(80008, mm.GuildScene.zhanBtn)
            end
        end
...
function BuZhenNewLayer:zhanBtnCbk(widget,touchkey)
    if touchkey == ccui.TouchEventType.ended then 
        ...
        print(" yyyyyyyyyyyyyyyyyy       mm.GuildId "..mm.GuildId)
        if mm.GuildId == 10036 then
            print(" yyyyyyyyyyyyyyyyyy       mm.data.curDuanWei "..mm.data.curDuanWei)
            mm:clearLayer()
            if tostring(mm.data.curDuanWei) == tostring(1093677105) then
                Guide:startGuildById(15051, mm.GuildScene.duanweiBtn)
            else
                Guide:GuildEnd()
            end
        end
...
function BuZhenNewLayer:saveFormationBack( event )
    ...
    if 0 == type then
        ...
        if mm.GuildId == 10204 and mm.GuildScene.nextFightTextBtn:isVisible() then
            performWithDelay(mm.GuildScene,function( ... )
                Guide:startGuildById(10205, mm.GuildScene.nextFightTextBtn)
            end, 0.01)
        elseif mm.GuildId == 10307 and mm.GuildScene.nextFightTextBtn:isVisible() then
            performWithDelay(mm.GuildScene,function( ... )
                Guide:startGuildById(10308, mm.GuildScene.nextFightTextBtn)
            end, 0.01)
        elseif mm.GuildId == 10407 and mm.GuildScene.nextFightTextBtn:isVisible() then
            performWithDelay(mm.GuildScene,function( ... )
                Guide:startGuildById(10408, mm.GuildScene.nextFightTextBtn)
            end, 0.01)
        elseif mm.GuildId == 10607 then
            Guide:GuildEnd()
        elseif mm.GuildId == 15003 then
            mm.GuildScene:anOnceNextFight()
            -- Guide:setHandVisible(false)
            -- Guide:setkuangImgVisible(false)
            -- Guide:setImageViewVisible(false)
            Guide:GuildEnd()
        elseif mm.GuildId == 15054 then
            -- mm.GuildScene:anOnceNextFight()
            -- Guide:setHandVisible(false)
            -- Guide:setkuangImgVisible(false)
            -- Guide:setImageViewVisible(false)
            cclog(" 保存阵型返回   "..mm.GuildId)
            Guide:GuildEnd()
        end

        if mm.GuildId == 80008 then
            Guide:GuildEnd()
        end
    else
        cclog(" 保存阵型返回 错误  ")
