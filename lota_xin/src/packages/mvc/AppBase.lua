...
function AppBase:run(initSceneName, tab)
    initSceneName = initSceneName or self.configs_.defaultSceneName
    self:enterScene(initSceneName, tab)
    initSceneNameGloble = initSceneName
end

function AppBase:push(sceneName, t, transition, time, more)
    local view = self:createView(sceneName, t)
    view:pushWithScene(transition, time, more)
    return view
end

function AppBase:pop()
    if director:getRunningScene() then
        director:popScene()
    end
end

function AppBase:enterScene(sceneName, tab, transition, time, more)
    local view = self:createView(sceneName, tab)
    view:showWithScene(transition, time, more)
    return view
end

function AppBase:createView(name, tab)
    for _, root in ipairs(self.configs_.viewsRoot) do
        local packageName = string.format("%s/%s", root, name)
        print("测试："..packageName)
        ...
        if status and (t == "table" or t == "userdata") then
            return view:create(self, name, tab)
        end
    end
